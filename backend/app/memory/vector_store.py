...
class SQLiteVectorStore:
    ...
    def _init_db(self):
        ...
        if self._has_function("vec_ann"):
            ...
            logger.warning("sqlite-vec extension is not installed, falling back to numpy for vector calculations.")
    ...
    def add(self, items: List[VectorItem], collection: str = "default"):
        ...
        conn.commit()
        logger.info(f"Added {len(items)} vectors to collection '{collection}'")
    ...
    def search(
        ...
        if self._has_function("vec_ann"):
            ...
            cursor.execute("SELECT 1")  # Dummy operation to maintain shape

        final_results = []
        for row, sim in results[:limit]:
            item = VectorItem(
                id=row["id"],
                content=row["content"],
            ...
            # If using vec_ann it would process `distance` here, but for fallback it's dot product based.
            final_results.append((item, sim))
    ...
    def clear(self, collection: str = "default"):
        ...
        conn.commit()
        logger.info("Vector store database cleared.")
